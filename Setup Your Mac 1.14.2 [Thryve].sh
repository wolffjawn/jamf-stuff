...
brandingIconLight="https://www.thryvedigital.com/images/logo__thryve--header-desktop.svg"
brandingIconDark="https://www.thryvedigital.com/images/logo__thryve--header-desktop.svg"
...
if [[ "${requiredMinimumBuild}" == "disabled" ]]; then
    ...
    # The Mac is running an operating system older than macOS 12 Monterey; exit with error
        ...
        osascript -e 'display dialog "Please advise your Support Representative of the following error:\r\rExpected macOS Build '${requiredMinimumBuild}' (or newer), but found macOS '${osVersion}' ('${osBuild}').\r\r" with title "Setup Your Mac: Detected Outdated Operating System" buttons {"Upgrade macOS"} with icon caution'
        updateScriptLog "PRE-FLIGHT CHECK: Executing /usr/bin/open '${outdatedOsAction}' …"
        su - "${loggedInUser}" -c "/usr/bin/open \"${outdatedOsAction}\""
        exit 1
...
if [ "$promptForAssetTag" == "true" ]; then
    assetTagJSON='{   "title" : "Asset Tag",
        ...
        "regex" : "^(ahn|AHN|osx|OSX|vm|VM)?[0-9]{6,}$",
...
function finalise(){
    ...
    if [[ "${jamfProPolicyTriggerFailure}" == "failed" ]]; then
        ...
        dialogUpdateSetupYourMac "progresstext: Complete! Please ${progressTextCompletionAction}enjoy your new ${modelName}!"
